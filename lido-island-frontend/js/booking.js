...
const STABLE_PROD_API = 'https://lido-island-backend.onrender.com';
const LEGACY_PROD_FALLBACK_API = 'https://backend-two-phi-89.vercel.app';
...
function getManualBlockedTablesByDate(date) {
    if (typeof window.getLidoManualBlockedTablesByDate === 'function') {
        return window.getLidoManualBlockedTablesByDate(date, BOOKING_CONFIG.TABLES.total);
    }
    return [];
}
...
function updateDisplay() {
    ...
    const umbrellaPrice = UMBRELLA_PRICE;
...
function validateBookingForm() {
    ...
    if (currentBooking.tables === 0 && currentBooking.chairs === 0 && Number(currentBooking.umbrellas || 0) === 0) {
        showError('Nessun articolo selezionato', 'Seleziona almeno un tavolo, una sdraio o un ombrellone');
        isValid = false;
    }
...
function handleBookingSubmit(e) {
    ...
    try {
        ...
        const umbrellasTotal = Number(booking.umbrellas || 0) * UMBRELLA_PRICE * 100;
        const total = Math.round(tablesTotal + chairsTotal + umbrellasTotal);
        if (total <= 0) {
            alert('Devi selezionare almeno un servizio per prenotare.');
            return;
        }
