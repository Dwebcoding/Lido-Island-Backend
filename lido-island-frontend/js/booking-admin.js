(function () {
  ...
  function resolveApiBase() {
    ...
    return 'https://lido-island-backend.onrender.com';
  }
  ...
  function getManualAdminRows(date) {
    if (date && typeof window.getLidoManualAdminBookingRows === 'function') {
      return window.getLidoManualAdminBookingRows(date);
    }
    if (!date && typeof window.getAllLidoManualAdminBookingRows === 'function') {
      return window.getAllLidoManualAdminBookingRows();
    }
    return [];
  }

  function getManualTableNumbers(manualRows) {
    return manualRows
      .flatMap((item) => Array.isArray(item.tableNumbers) ? item.tableNumbers : [])
      .map((value) => Number(value))
      .filter((value) => Number.isInteger(value) && value > 0)
      .sort((left, right) => left - right);
  }

  function renderManualBlocks(date) {
    const manualRows = getManualAdminRows(date);
    const manualTables = getManualTableNumbers(manualRows);
    const message = document.getElementById('manualBlocksMessage');
    const list = document.getElementById('manualBlocksList');

    if (!date) {
      setText('statsManualTables', 'Seleziona una data');
      if (message) message.textContent = 'Seleziona una data per visualizzare i tavoli bloccati manualmente.';
      if (list) list.innerHTML = '';
      return;
    }

    setText('statsManualTables', `${manualTables.length} / 110`);

    if (!message || !list) return;

    if (!manualTables.length) {
      message.textContent = 'Nessun tavolo bloccato manualmente per la data selezionata.';
      list.innerHTML = '';
      return;
    }

    message.textContent = `${manualTables.length} tavoli bloccati manualmente per ${date}.`;
    list.innerHTML = manualTables.map((tableNumber) => `
      <span class="id-pill">Tavolo ${tableNumber}</span>
    `).join('');
  }
  ...
  function getSortValue(item, sortBy) {
    if (sortBy === 'oldest') return new Date(item.paidAt || item.createdAt || `${item.date || '1970-01-01'}T00:00:00`).getTime();
    if (sortBy === 'event_asc') return new Date(`${item.date || '1970-01-01'}T00:00:00`).getTime();
    if (sortBy === 'event_desc') return -new Date(`${item.date || '1970-01-01'}T00:00:00`).getTime();
    if (sortBy === 'amount_asc') return Number(item.amount || 0);
    if (sortBy === 'amount_desc') return -Number(item.amount || 0);
    return -new Date(item.paidAt || item.createdAt || `${item.date || '1970-01-01'}T00:00:00`).getTime();
  }

  function sortBookings(items, sortBy) {
    return [...items].sort((left, right) => {
      const leftValue = getSortValue(left, sortBy);
      const rightValue = getSortValue(right, sortBy);
      if (leftValue === rightValue) {
        return String(left.bookingId || left.id || '').localeCompare(String(right.bookingId || right.id || ''));
      }
  ...
  async function loadBookings(e) {
    ...
    try {
      ...
      const manualItems = getManualAdminRows(date).filter((item) => !status || String(item.status || '').toLowerCase() === status.toLowerCase());
      const manualTablesCount = getManualTableNumbers(manualItems).length;
      const items = sortBookings([...apiItems, ...manualItems], sortBy);
      ...
      if (statsRes && statsRes.ok) {
        ...
        setText('statsBookings', String((stats.bookings || 0) + manualItems.length));
        setText('statsTables', `${(stats.tablesBooked || 0) + manualTablesCount} / 110`);
